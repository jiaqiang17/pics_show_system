...
<template>
  ...
  <el-input v-model="searchInfo.tagName" placeholder="搜索条件" />
</el-form-item>
    ...
    <div class="gva-table-box">
        <div class="gva-btn-list">
            ...
            <ExportTemplate  template-id="nail_NailTag" />
            <ExportExcel  template-id="nail_NailTag" filterDeleted/>
            <ImportExcel  template-id="nail_NailTag" @on-success="getTableData" />
        </div>
        <el-table
        ref="multipleTable"
        style="width: 100%"
        ...
        <el-table-column sortable align="left" label="日期" prop="CreatedAt" width="180">
            ...
            <el-table-column align="left" label="标签名称" prop="tagName" width="120" />

            <el-table-column align="left" label="标签颜色" prop="tagColor" width="120" />

            <el-table-column sortable align="left" label="排序号" prop="sort" width="120" />

            <el-table-column align="left" label="状态" prop="status" width="120">
    <template #default="scope">
    {{ filterDict(scope.row.status,nail_tag_statusOptions) }}
    </template>
</el-table-column>
        <el-table-column align="left" label="操作" fixed="right" :min-width="appStore.operateMinWith">
            <template #default="scope">
            <el-button  type="primary" link class="table-button" @click="getDetails(scope.row)"><el-icon style="margin-right: 5px"><InfoFilled /></el-icon>查看</el-button>
            <el-button  type="primary" link icon="edit" class="table-button" @click="updateNailTagFunc(scope.row)">编辑</el-button>
    ...
    {{ detailForm.tagName }}
                    ...
                    <el-descriptions-item label="标签图标">
    {{ detailForm.tagIcon }}
</el-descriptions-item>
...
const getTableData = async() => {
  const table = await getNailTagList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteNailTagByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateNailTagFunc = async(row) => {
    const res = await findNailTag({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteNailTagFunc = async (row) => {
    const res = await deleteNailTag({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
